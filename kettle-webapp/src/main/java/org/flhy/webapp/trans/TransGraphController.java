...
public class TransGraphController {
	...
	protected void save(@RequestParam String graphXml) throws Exception {
		...
		newsaveFile("e:\\a01.ktr",graphXml);
	...
	public  void newsaveFile(String fileName, String transXml) {
...
//	private static HashMap<String, TransExecutor> executions = new HashMap<String, TransExecutor>();
	...
	@ResponseBody
	@RequestMapping(method = RequestMethod.POST, value = "/getProcedures")
	protected void getProcedures(@RequestParam String graphXml, @RequestParam String databaseName, @RequestParam String schema, @RequestParam String table) throws Exception {
		GraphCodec codec = (GraphCodec) PluginFactory.getBean(GraphCodec.TRANS_CODEC);
		TransMeta transMeta = (TransMeta) codec.decode(graphXml);
		DatabaseMeta inf = transMeta.findDatabase(databaseName);
