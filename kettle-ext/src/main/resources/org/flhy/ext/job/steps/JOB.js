JobEntryJOB = Ext.extend(KettleTabDialog, {
	...
	initComponent: function() {
		...
		var wJobname = new Ext.form.TextField({flex: 1, disabled: true,runType:'JOB'});
		var wbTransname = new Ext.Button({text: '选择..', disabled: true, handler: function() {
		//	alert(1111);
				var runtype = 'jOB';
			var object = "<input type =\"hidden\" id=\"runType\" value=\"JOB\" />" ;
			var dialog = new RepositoryExplorerWindow();

			dialog.on('ok', function(dir, name,runtype) {

				//console.log(me);

				alert(name);
				wDirectory.setValue(dir);
				wJobname.setValue(name);
				me.setStepname(name);
				console.log(name);
				dialog.close();
			});
